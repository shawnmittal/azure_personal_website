...
df = pd.read_csv(current_dir / "titanic.csv", header=0, sep="\t")
...
def redraw(p_class):
    ...
    return (survived_chart, title_chart, hist_age)
...
def class_titles_bar_chart(dataset, pass_class, cpalette=palette):
    ...
    hover_tool = HoverTool(tooltips=[("Title", "@titles"), ("Count", "@values")])
...
def age_hist(dataset, pass_class, color=palette[1]):
    hist, edges = np.histogram(
        dataset[dataset["Pclass"] == int(pass_class)]["Age"].fillna(df["Age"].mean()),
        bins=25,
    )
